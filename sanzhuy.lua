...
createToggle(MainPage, "Auto Rod", function(v)
    _G.AutoRod = v
end)
...
function SetTotemStatus(text, color)
    ...
    if color then
        TotemStatus.TextColor3 = color
    else
        TotemStatus.TextColor3 = Color3.fromRGB(255,255,255)
    end
end
...
RefreshTotemBtn.TextSize = 12
RefreshTotemBtn.BorderSizePixel = 0
RefreshTotemBtn.Text = "Refresh Totem Status"

Instance.new("UICorner", RefreshTotemBtn).CornerRadius = UDim.new(0,8)
...
task.spawn(function()
    while _G.FishItHubLoaded do
        ...
        if F.TotemCooldown > 0 then
            SetTotemStatus("Cooldown: "..F.TotemCooldown.."s", Color3.fromRGB(255,200,0))
            F.TotemCooldown -= 1
            continue
        end
...
if char then
    ...
    if not tool or not tool.Name:lower():find("rod") then
        ...
        -- RETRY WAIT
        if F.RetryWait then
            local s = math.floor(F.RetryWait - os.clock())
            if s > 0 then
                SetTotemStatus("Retry in "..s.."s", Color3.fromRGB(255,150,0))
                continue
            else
                F.RetryWait = false
            end
        ...
        -- REQUEST PASANG TOTEM
        SetTotemStatus("Placing Totem...", Color3.fromRGB(80,160,255))
        SpawnTotem:FireServer(uuid)

        -- tunggu server respon (script 1 pakai 0.5)
        task.wait(0.7)

        -- kalau server TIDAK BALAS → retry 5 menit
        if F.TotemCooldown <= 0 then
            SetTotemStatus("Failed! Retry in 5 min", Color3.fromRGB(255,120,0))
            F.RetryWait = os.clock() + TRY_INTERVAL
        end
    end
end)
