...
------------------------------------------------------
-- AUTO TOTEM SYSTEM (100% MIRIP SCRIPT 1)
------------------------------------------------------
...
-- ambil uuid totem (sama persis script 1)
...
------------------------------------------------------
-- TOTEM SPAWNED EVENT (SERVER CONFIRMATION)
------------------------------------------------------
TotemSpawned.OnClientEvent:Connect(function()
    ...
    SetTotemStatus("Cooldown: 3600s", Color3.fromRGB(255,200,0))

    -- equip rod setelah pasang totem
...
------------------------------------------------------
-- AUTO EQUIP ROD LOOP (script 1 punya loop ringan)
------------------------------------------------------
task.spawn(function()
    while _G.FishItHubLoaded do
        task.wait(2.5)
        if F.AutoTotem then
            pcall(EquipRodStatus)
        end
    end
end)

------------------------------------------------------
-- AUTO TOTEM LOOP (MAIN LOGIC)
------------------------------------------------------
task.spawn(function()
    while _G.FishItHubLoaded do
        task.wait(1)
        ...
        -- COOLDOWN
        if COOLDOWN > 0 then
            SetTotemStatus("Cooldown: " .. COOLDOWN .. "s", Color3.fromRGB(255,200,80))
            COOLDOWN -= 1
            continue
        end
        ...
        -- RETRY MODE
        if retryWait then
            local remain = retryWait - os.clock()
            if remain > 0 then
                SetTotemStatus("Retry in: "..math.floor(remain).."s", Color3.fromRGB(255,180,0))
                continue
            else
                retryWait = false
            end
        ...
        if not uuid then
            SetTotemStatus("No Totem Found", Color3.fromRGB(255,60,60))
            task.wait(4)
            continue
        end

        -- MULAI PASANG TOTEM
        SetTotemStatus("Placing Totem...", Color3.fromRGB(100,160,255))
        ...
        -- cek apakah sukses
        if not success then
            SetTotemStatus("Failed! Retry in 5 min", Color3.fromRGB(255,150,0))
...
createToggle(VisualPage, "Reduce Map", function(state)
    if state then
        ...
        Notify("Reduce Map: ON (Executed 1x)", Color3.fromRGB(0,255,150))
    else
        Notify("Reduce Map: OFF (cannot restore map)", Color3.fromRGB(255,80,80))
    end
end)
...
createToggle(VisualPage, "Performance Mode", function(state)
    if state then
        -- TRUE ONE CLICK ULTRA LOW GRAPHICS
        pcall(function()
            settings().Rendering.QualityLevel = Enum.QualityLevel.Level01
        end)
        ...
        Notify("Performance Mode: ON (Ultra Low Graphics)", Color3.fromRGB(0,255,120))
    else
        Notify("Performance Mode: OFF (cannot restore lighting)", Color3.fromRGB(255,80,80))
    end
end)
...
createToggle(VisualPage, "Extreme Potato Mode", function(state)
    ...
    Notify("Extreme Potato Mode: ACTIVATED âš¡", Color3.fromRGB(255,200,50))
    ...
    -- ðŸ”¥ DELETE DECALS & TEXTURES (SUPER FPS BOOST)
    task.spawn(function()
        for _,obj in ipairs(workspace:GetDescendants()) do
    ...
    Notify("ðŸ”¥ Extreme Potato Mode Applied â€” MAX FPS!", Color3.fromRGB(0,255,120))
end)
